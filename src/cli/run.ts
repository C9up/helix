...
async function runOnce(config: RunConfig): Promise<RunOutcome> {
	...
	let coverageViolations: ThresholdViolation[] | undefined;
	let diffCoverage: DiffSummary | undefined;
	let diffCoverageViolations: ThresholdViolation[] | undefined;
	...
	// Diff-cov requires the full-tree coverage to be enabled — refuse
	// upfront so the user fixes the config rather than getting a silent
	// no-op.
	if (config.diffCoverage?.enabled && !coverageEnabled) {
		throw new Error(
	...
	if (config.diffCoverage?.enabled && config.diffCoverage.cwd) {
	...
	try {
		...
		if (coverageEnabled && session && config.coverage) {
			try {
				...
				coverage = finalised.summary;
				coverageViolations = finalised.violations;
				...
				if (config.diffCoverage?.enabled && coverage) {
					try {
						const diff = await finaliseDiff({
							...
							outputDir:
								config.diffCoverage.outputDir ?? config.coverage.outputDir,
							cwd: config.diffCoverage.cwd,
						});
						if (diff.warning) {
							process.stderr.write(`${diff.warning}\n`);
						} else {
							diffCoverage = diff.summary;
							diffCoverageViolations = diff.violations;
							if (diff.textReport) process.stdout.write(diff.textReport);
							if (diff.violations.length > 0) {
								process.stderr.write(
									`${diffViolationSummary(diff.violations)}\n`,
								);
							}
						}
					} catch (err) {
						...
						// Don't let a thrown finaliseDiff masquerade as a clean run —
						// the user explicitly opted into diff-cov and a CI gate
						// configured around it must still fail.
						diffCoverageFailed = true;
					}
				}
			} catch (err) {
				// Coverage finalisation failed (disk full, permission, etc.) —
				// the test summary already printed; surface the failure but
				// don't crash the whole run with a stack trace.
				process.stderr.write(
		...
		const coverageExit =
			coverageViolations && coverageViolations.length > 0 ? 1 : 0;
		const diffCoverageExit =
			(diffCoverageViolations && diffCoverageViolations.length > 0) ||
			diffCoverageFailed
				? 1
				: 0;
		return {
